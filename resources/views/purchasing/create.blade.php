        ...
        <form action="{{ route('purchasing.store') }}" method="POST" autocomplete="off">
            ...
            <div class="card mb-3 p-3">
                <div class="row g-3">
                    ...
                    {{-- GANTI: Metode Pembayaran -> Operator/PIC (employees bagian operasional) --}}
                    <div class="col-md-3">
                        <label class="form-label">Operator / PIC</label>
                        <select class="form-select" name="operator_id" id="operatorSelect" required>
                            <option value="">Pilih Operator...</option>
                            @foreach ($operators as $op)
                                <option value="{{ $op->id }}"
                                    {{ (string) old('operator_id') === (string) $op->id ? 'selected' : '' }}>
                                    {{ $op->name }}
                                </option>
                            @endforeach
                        </select>
                        <div class="form-text muted">Sumber data: employees (bagian operasional)</div>
                    </div>
                </div>
            </div>
            ...
            <div class="row g-3">
                <div class="col-lg-7">
                    <div class="card h-100 p-3">
                        {{-- PINDAH: OTENG/Ongkir ke atas Catatan --}}
                        <label class="form-label">OTENG / Ongkir</label>
                        {{-- tampilan --}}
                        <input type="text" inputmode="numeric" class="form-control text-end mb-3"
                            id="other_costs_display" value="{{ old('other_costs', 0) }}">
                        {{-- server --}}
                        <input type="hidden" name="other_costs" id="other_costs" value="{{ old('other_costs', 0) }}">
                        <div class="form-text muted mb-3">Masuk HPP invoice</div>
                            ...
                            <div class="row mt-2 g-2">
                                ...
                                <div class="col-6">
                                    <select class="form-select form-select-sm" name="paid_account">
                                        ...
                                        <option value="SEABANK" {{ $acc === 'SEABANK' ? 'selected' : '' }}>SeaBank
                                        </option>
                                        <option value="TRANSFER" {{ $acc === 'TRANSFER' ? 'selected' : '' }}>Transfer
                                        </option>
                                    </select>
...
@push('scripts')
    <script>
        ...
        function addRow(prefill = null) {
            ...
            const openMenu = (list) => {
                menu.innerHTML = list.length ?
                    list.map((it, idx) => `<div class="suggest-item ${idx===activeIdx?'active':''}" data-index="${idx}">
                    <span class="label">${it.name}</span><span class="code">${it.code||''}</span></div>`).join('') :
        ...
        $form.addEventListener('submit', (e) => {
            ...
            // 2) buang baris kosong, isi item_name jika perlu
...
@push('scripts')
    <script>
        ...
        /* ===== Harga terakhir helper (BARU) ===== */
        function getLastPriceFor(itemId, supplierId) {
            if (!itemId) return null;
            const bySup = (supplierId && LAST_BY_SUP[supplierId]) ? LAST_BY_SUP[supplierId][itemId] : null;
            const any = LAST_PRICES[itemId] ?? null;
            return (bySup ?? any) ?? null;
        }

        function refreshRowLastPrice(tr) {
            const lastTxt = tr.querySelector('.lastPriceText');
            const itemId = parseInt(tr.querySelector('.itemIdInput')?.value || '', 10);
            if (!lastTxt) {
                return;
            }
            const supId = activeSupplierId();
            const v = getLastPriceFor(itemId, supId);
            lastTxt.textContent = (v && v > 0) ? rupiah(v) : 'Belum ada';
        }

        function refreshAllRowsLastPrice() {
            $tbody.querySelectorAll('tr:not(.empty-state)').forEach(tr => refreshRowLastPrice(tr));
        }
        ...
        function addRow(prefill = null) {
            ...
            input.addEventListener('input', () => {
                ...
                hidItemName.value = '';
                hidUnit.value = 'pcs';
                // reset teks harga terakhir
                const lastTxt = tr.querySelector('.lastPriceText');
                if (lastTxt) lastTxt.textContent = '—';
            });
            input.addEventListener('keydown', (e) => {
                if (!isMobile && e.key === 'F2') {
                    e.preventDefault();
        ...
        /* === Supplier change: update classId + last prices SEMUA BARIS (BARU) === */
        function handleSupplierChange() {
            const ok = !!activeSupplierClassId();
            $btnAdd.disabled = !ok;
            if (!ok) {
            ...
            setClassIdOnAllRows(); // pastikan class id baris ikut supplier
            refreshAllRowsLastPrice(); // <— update harga terakhir tiap baris sesuai supplier baru
            if (!$tbody.querySelector('tr:not(.empty-state)')) addRow();
        }
        ...
        (function restoreIfAny() {
            if (OLD_LINES && OLD_LINES.length) {
                ...
                OLD_LINES.forEach(line => addRow(line));
                setClassIdOnAllRows();
                refreshAllRowsLastPrice(); // jaga2 saat reload form setelah error
                recalcSummary();
            } else {
                handleSupplierChange();
            }
